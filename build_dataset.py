...
# then initialize the list of image paths and labels
...
# loop over the rows
for row in rows:
    # unpack the row, then update the image paths and label list
...
# compute the number of images that should be used for validation and testing
numVal = int(len(trainPaths) * config.NUM_VAL_IMAGES)
numTest = int(len(trainPaths) * config.NUM_TEST_IMAGES)

# class labels represented as strings so we need to encode them
...
print("[INFO] construct tesing data...")
